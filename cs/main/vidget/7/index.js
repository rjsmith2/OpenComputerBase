...
f.socket=function(){
    ...
    consoleSocket.onmessage=function(eve){
		...
		var from=innerJSON.from;
    	if(doFunction[command])doFunction[command](json);
		if(this.fz.listeners[from]){
			console.log("dispatching");
				this.fz.listeners[from](json);
		}
     }
...
var windowOptions=[
	...
	featureWindow[address].address=address;
	win.resizeTo(700,500);
	var document=win.document;
	 ...
	 executeCode.addEventListener("click",function(){
		var address=this[0].address.toString();
		var parentWindow=this[0];
		var f=this[1];
		eval(this[0].currentCode);
	 }.bind([featureWindow[address],f]));
